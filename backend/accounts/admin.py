...
class CustomUserAdmin(BaseUserAdmin):
    ...
    list_display = (
        'email', 'first_name', 'last_name', 'phone_number', 'city', 'is_staff',
        'is_active', 'role', 'is_verified', 'blocked_until', 'block_count',
        'is_permanently_banned', 'auth_provider'
    )

    list_filter = (
        'is_active', 'is_staff', 'is_superuser', 'auth_provider', 'is_verified', 'role'
    )

    readonly_fields = (
        'last_login', 'created_at', 'updated_at', 'blocked_until',
        'block_count', 'is_permanently_banned'
    )
