...
const Cart = () => {
  ...
  const getImageUrl = (variant) => {
    if (!variant) return "/placeholder.png";
    if (variant.images?.length) return variant.images[0].url;
    if (variant.product_images?.length)
      return variant.product_images[0].startsWith("http")
        ? variant.product_images[0]
        : `${axiosInstance.defaults.baseURL || ""}${variant.product_images[0]}`;
            ...
            {cartItems.map((item) => {
              ...
              return (
                ...
                >
                  <img
                    src={imageUrl}
                    alt={variant?.product_name || "Product"}
                    className="w-20 h-20 object-cover rounded-md mb-4 md:mb-0 md:mr-6"
                  />
                  <div className="flex-1">
                    <h2 className="text-lg font-semibold">
                      {variant
                        ? `${variant.product_name} - ${variant.variant_name || "Default"}`
                        : "Product unavailable"}
                    </h2>
                    ...
                    <p className="text-sm text-gray-600">
                      Subtotal: ₹{((variant?.price || 0) * item.quantity).toFixed(2)}
                    </p>
                  ...
                  {variant && (
                    <div className="flex flex-col md:flex-row items-center mt-4 md:mt-0 space-y-2 md:space-y-0 md:space-x-2">
                      ...
                      <button
                        ...
                        className="ml-0 md:ml-4 px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700"
                      >
                        Remove
                      </button>
                    </div>
              ...
              <button
                onClick={() => {
                  if (isAuthenticated) {
                    ...
                    toast.info(
                      "Please log in as a customer to proceed to checkout"
                    );
